...
export const GameContainer: React.FC<{
  ...
  const updateActionableTerritories = () => {
    ...
    const buildableTerritories = gameState.players[gameState.currentPlayer].territories.filter(territoryId => canBuildOnTerritory(territoryId));

    // Get territories where units can be recruited (owned, has barracks, has fewer than 2 unit types)
    const recruitableTerritories = gameState.players[gameState.currentPlayer].territories.filter(territoryId => canRecruitInTerritory(territoryId));
    ...
    const newErrorMessages = {
      attack: hasEnemyAdjacentTerritories() ? gameState.players[gameState.currentPlayer].units.length > 0 ? "Select your territory first" : "No military units" : "No enemy territories nearby",
      recruit: recruitableTerritories.length > 0 ? "Select territory with barracks" : "Need barracks with space for units",
      build: buildableTerritories.length > 0 ? "Select territory to build on" : "No available territories",
      expand: hasResourcesForExpansion() ? expandableTerritories.length > 0 ? "Select territory to expand to" : "No available territories" : "Need 100 Gold and 20 Food"
    };
  ...
  const generateTerritories = (boardSize: number): Territory[] => {
    ...
    for (const territory of territories) {
      ...
      for (const dir of directions) {
        const adjX = x + dir.x;
        const adjY = y + dir.y;
        const adjTerritory = territories.find(t => t.position.x === adjX && t.position.y === adjY);
        if (adjTerritory) {
  ...
  const generateResourcesForType = (type: string): {
    gold: number;
    wood: number;
    stone: number;
    food: number;
  ...
  const handleRecruitUnit = (unitType: string) => {
    ...
    switch (unitType) {
      ...
      case "artillery":
        ...
        canRecruit = currentPlayer.resources.food >= 60 && currentPlayer.resources.gold >= 50 && currentPlayer.resources.wood >= 30 && currentPlayer.resources.stone >= 20;
        break;
    }
    if (!canRecruit) {
  ...
  const handleAttackTerritory = (defendingTerritoryId: number) => {
    ...
    const attackingUnits = attackingPlayer.units.filter(unit => attackingTerritory.units.includes(unit.id));
    ...
    const defendingUnits = defendingPlayer.units.filter(unit => defendingTerritory.units.includes(unit.id));
    ...
    updatedPlayers[attackerIndex].units = updatedPlayers[attackerIndex].units.filter(u => attackerUnitIds.has(u.id) || u.territoryId !== selectedTerritory);
    ...
    updatedPlayers[defenderIndex].units = updatedPlayers[defenderIndex].units.filter(u => defenderUnitIds.has(u.id) || u.territoryId !== defendingTerritoryId);
    ...
    if (attackerWins && updatedDefendingUnits.length === 0) {
      ...
      updatedPlayers[defenderIndex].territories = updatedPlayers[defenderIndex].territories.filter(t => t !== defendingTerritoryId);
      ...
      updatedTerritories[defendingTerritoryIndex].buildings.forEach(buildingId => {
        ...
        if (building) {
          ...
          updatedPlayers[defenderIndex].buildings = updatedPlayers[defenderIndex].buildings.filter(b => b.id !== buildingId);
  ...
  const renderPositionedMenu = () => {
    ...
    return <div className="absolute z-30 pointer-events-auto" style={{
  ...
  const renderPhase = () => {
    if (gameState.phase === "setup") {
      return <div className="absolute top-0 left-0 w-full p-4 bg-gray-800 bg-opacity-75 text-white text-center z-10">
  ...
  const renderGameOver = () => {
    if (gameState.gameOver) {
      return <div className="absolute inset-0 bg-black bg-opacity-75 flex items-center justify-center z-20">
  ...
  const renderResourceGainToast = () => {
    if (showResourceGain && gameState.lastResourceGain) {
      ...
      return <div className="absolute top-16 right-4 bg-gray-800 bg-opacity-90 text-white p-4 rounded-lg shadow-lg z-30 animate-fadeIn">
  ...
  return <div className="relative h-full w-full flex flex-col">
      <GameTopBar turn={gameState.turn} currentPlayer={gameState.currentPlayer} playerName={gameState.players[gameState.currentPlayer].name} playerColor={gameState.players[gameState.currentPlayer].color} onExitGame={onExitGame} phase={gameState.phase} />
      
      <div className="flex flex-1 overflow-hidden">
        <div className="flex-1 relative" ref={mapContainerRef}>
          <GameBoard territories={gameState.territories} players={gameState.players} selectedTerritory={selectedTerritory} onTerritorySelect={handleTerritorySelect} onClaimTerritory={handleClaimTerritory} onAttackTerritory={handleAttackTerritory} currentPlayer={gameState.currentPlayer} phase={gameState.phase} actionTaken={gameState.actionTaken} expandableTerritories={gameState.expandableTerritories} attackableTerritories={gameState.attackableTerritories} buildableTerritories={gameState.buildableTerritories} recruitableTerritories={gameState.recruitableTerritories} currentAction={gameState.currentAction} actionsPerformed={gameState.actionsPerformed} />
        ...
        {!isMobile && <div className="w-64 p-4 flex flex-col bg-gray-800">
            <ResourceDisplay resources={gameState.players[gameState.currentPlayer].resources} resourceGain={gameState.lastResourceGain} />
            
            {gameState.phase === "playing" && <GameControls onBuildClick={() => handleMenuSelect("build")} onRecruitClick={() => handleMenuSelect("recruit")} onExpandClick={() => handleMenuSelect("expand")} onAttackClick={() => handleMenuSelect("attack")} onEndTurnClick={handleEndTurn} disabled={gameState.actionTaken} actionTaken={gameState.actionTaken} expandMode={gameState.currentAction === "expand"} attackMode={gameState.currentAction === "attack"} canAttack={gameState.players[gameState.currentPlayer].units.length > 0 && hasEnemyAdjacentTerritories()} hasResourcesForExpansion={hasResourcesForExpansion()} canRecruit={gameState.players[gameState.currentPlayer].territories.some(territoryId => canRecruitInTerritory(territoryId))} canBuild={gameState.players[gameState.currentPlayer].territories.length > 0} actionsPerformed={gameState.actionsPerformed} errorMessages={errorMessages} />}
            
            <GameMenus onInfoButtonClick={type => setActiveInfoModal(type)} />
          </div>}
      </div>
      
      {isMobile && gameState.phase === "playing" && <div className="bg-gray-100 p-2">
          <GameControls onBuildClick={() => handleMenuSelect("build")} onRecruitClick={() => handleMenuSelect("recruit")} onExpandClick={() => handleMenuSelect("expand")} onAttackClick={() => handleMenuSelect("attack")} onEndTurnClick={handleEndTurn} disabled={gameState.actionTaken} actionTaken={gameState.actionTaken} expandMode={gameState.currentAction === "expand"} attackMode={gameState.currentAction === "attack"} canAttack={gameState.players[gameState.currentPlayer].units.length > 0 && hasEnemyAdjacentTerritories()} hasResourcesForExpansion={hasResourcesForExpansion()} canRecruit={gameState.players[gameState.currentPlayer].territories.some(territoryId => canRecruitInTerritory(territoryId))} canBuild={gameState.players[gameState.currentPlayer].territories.length > 0} actionsPerformed={gameState.actionsPerformed} errorMessages={errorMessages} />
        </div>}
      
      {/* For mobile, make the build/recruit menus appear as modals */}
      {isMobile && activeMenu && selectedTerritory !== null && <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-gray-800 p-4 rounded-lg w-11/12 max-w-md">
            <div className="flex justify-between items-center mb-2">
              <h3 className="text-white font-bold">
                {activeMenu === "build" ? "Build Structure" : "Recruit Unit"}
              </h3>
              <Button variant="ghost" size="sm" onClick={() => setActiveMenu(null)} className="text-gray-300 hover:text-white">
                ×
              </Button>
            </div>
